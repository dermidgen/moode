 ...
 *	- set volume knob to readonly if rotary encoder enabled
 *
 *	TC (Tim Curtis) 2015-09-05: r2.3
 *	- add volume_mixer_name
 *
 *	TC (Tim Curtis) 2015-10-30: r2.4
 *	- add playlist-display to tcmods.conf
 *
 */
...
jQuery(document).ready(function($) { 'use strict';
	...
	// TC (Tim Curtis) 2015-08-30: set volume knob to readonly if rotary encoder enabled
	if (TCMCONF.json['volume_mixer_type'] == "disabled" || TCMCONF.json['rotary_encoder_enabled'] == "1") {
		...
		// TC (Tim Curtis) 2015-08-30: for rotary encoder "Yes", ignored by MPD if mixer type 'disabled' since MPD sets vol to -1
	    sendMpdCmd('setvol 0');
		...
		// // TC (Tim Curtis) 2015-08-30: needed ?
		//setVolume(TCMCONF.json['volume_knob_setting'], '');
	}
    ...
    $('.playlist').on('click', '.pl-entry', function() {
        ...
        sendMpdCmd(cmd);
        GUI.halt = 1;
        //$('.playlist li').removeClass('active'); //TC TEST
