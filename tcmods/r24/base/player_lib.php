 ...
 *	- add Audiophonics PCM5122 DAC, Mamboberry HiFi DAC+, Lucid Labs Raspberry Pi DAC, PlainDAC, PlainDAC+ and HifiBerry DAC+ Pro
 ...
 *	- add playlist_display to tcmods.conf
 *
 */
...
function loadAllLib($sock) {
	...
	$debug_flags = str_replace("\n", '', explode(',', file_get_contents("/var/www/debug.conf")));
...
// TC (Tim Curtis) 2015-10-30: add year 2000 for debug 3
function _loadDirForLib($sock, $flat, $dir, $debug_flags) { // remove $dir in final version...
	if ($debug_flags[4] == "1") {
		...
		sendMpdCommand($sock, "find modified-since 1901-01-01T00:00:00Z"); // full time stamp, year 1901
	} else if ($debug_flags[4] == "3") {
		sendMpdCommand($sock, "find modified-since 2000-01-01T00:00:00Z"); // full time stamp, year 2000
	} else {
...
// TC (Tim Curtis) 2015-08-30: replaced by new version from Andreas Goetz 
...
function addQueue($sock,$path) {
	...
	//  TC (Tim Curtis) 2015-10-30: add cue extension
...
function _parseFileListResponse($resp) {
	if ( is_null($resp) ) {
		...
		while ( $plistLine ) {
			...
			// TC (Tim Curtis) 2015-10-30: handle cue sheet files, treat as file and not playlist
...
function _parseStatusResponse($resp) {
	if ( is_null($resp) ) {
		...
		// TC (Tim Curtis) 2015-10-30: workaround for AAC files that show "f" for bit depth, assume decoded to 24 bit
...
function wrk_mpdconf($outpath, $db, $kernelver, $i2s) {
	...
	foreach ($mpdcfg as $cfg) {
		...
		// TC (Tim Curtis) 2015-10-30: turn off logging	
...
// TC (Tim Curtis) 2015-10-30: add limit 2 to explode to avoid incorrect parsing where string contains more than one "+"
...
function _updTcmodsConf($tcmconf) {
	...
	$data .= 'playlist_display: '.$tcmconf['playlist_display']."\n";
...
// TC (Tim Curtis) 2015-10-30: add Audiophonics PCM5122 DAC, Lucid Labs Raspberry Pi DAC, PlainDAC, PlainDAC+ and HifiBerry DAC+ Pro
// TC (Tim Curtis) 2015-10-30: automatically update audio device fields in tcmods.conf
function _setI2sDtoverlay($db, $device) {
	...
	if ($device == 'I2S Off') {
		...
		switch ($device) {
			...
			case 'Lucid Labs Raspberry Pi DAC':
			case 'PlainDAC':
			case 'Mamboberry HiFi DAC+':
				sysCmd('echo dtoverlay=hifiberry-dac >> /boot/config.txt');
				break;
			case 'HiFiBerry DAC+':
			case 'HiFiBerry DAC+ Pro':
			case 'PlainDAC+':
...
// TC (Tim Curtis) 2015-10-30: add check for kernel 4.1.10+
function getMixerName($kernelver, $i2s) {
	if ($i2s != 'I2S Off') {
		if ($i2s == 'HiFiBerry Amp(Amp+)') {
			$mixername = 'Master'; // Hifiberry Amp(Amp+) i2s device 
		} else {				
			if ($kernelver == '3.18.11+' || $kernelver == '3.18.14+' || $kernelver == '4.1.10+') {
